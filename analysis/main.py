...
def _parse_payload_data(payload, offset):
    # Parses [IP len u8][IP][Request len u16][Request]
    ip_len = struct.unpack('>B', payload[offset:offset + 1])[0]
    offset += 1
    ip_address = payload[offset:offset + ip_len].decode('utf-8', errors='replace')
    offset += ip_len
    ...
    try:
        while True:
            ...
            if msg_type == 1:  
                ...
                if threat_id == 255: continue  # NO_THREAT flag bypass
                ...
                event_queue.put(Event(
                    ...
                    endpoint=request_str, metadata={"threat_id": threat_id}
                ))
                
            # Type 4: EVENT
            elif msg_type == 4:  
                event_id = struct.unpack('>B', payload[0:1])[0]
                threat_id = struct.unpack('>B', payload[1:2])[0] 
                ...
                event_queue.put(Event(
                    ...
                    endpoint=request_str, metadata={"event_id": event_id, "threat_id": threat_id}
                ))
